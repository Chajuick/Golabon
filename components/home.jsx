// 홈 화면 섹션 — 원본 comp2에서 이식. go() → next/link, 카드 찜은 ProductCard 내부 처리
import Link from 'next/link';
import { Icon } from './icons';
import { Verdict, Stars, ProductImg, fmt } from './ui';
import { ProductCard } from './ProductCard';
import { GOLABON_DATA as DATA, GB } from '@/lib/data';

function Hero() {
  const feat = GB.featured();
  const isPlace = feat?.type === 'place';
  const facts = !feat
    ? []
    : isPlace
      ? [
          ...
          { k: '가격대', v: feat.priceShort, c: 'var(--coral-deep)', bg: 'var(--cream)' },
        ]
      : [
          { k: '직접 사용', v: feat.usedDays + '일', c: 'var(--green-deep)', bg: 'var(--green-tint)' },
          { k: '재구매 의향', v: feat.reBuy, c: 'var(--ink)', bg: 'var(--cream)' },
          { k: '최저가', v: fmt(feat.priceFrom) + '원~', c: 'var(--coral-deep)', bg: 'var(--cream)' },
        ];
  return (
    <section style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(180deg, #fff 0%, var(--cream) 100%)', borderBottom: '1px solid var(--line)' }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(110% 80% at 92% 0%, rgba(31,158,120,.07), transparent 60%)' }} />
      <div className="wrap" style={{ position: 'relative', padding: '40px 24px 30px' }}>
        <div className="gb-hero-grid" style={{ display: 'grid', gridTemplateColumns: feat ? '1.15fr .85fr' : '1fr', gap: 36, alignItems: 'center' }}>
          <div className="fade-in">
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 28, flexWrap: 'wrap' }}>
              <Link className="btn btn-green btn-lg" href="/category">
                검증된 추천 보기 <Icon name="arrowR" size={18} />
              </Link>
              <Link className="btn btn-ghost btn-lg" href="/rejected">
                <Icon name="thumbDn" size={17} /> 비추천·탈락 먼저 보기
              </Link>
            </div>
          </div>

          {feat && <div className="gb-hero-card fade-in" style={{ animationDelay: '.08s' }}>
            <div className="card" style={{ boxShadow: 'var(--sh-lg)', borderColor: 'transparent', overflow: 'visible' }}>
              <div style={{ position: 'relative', borderRadius: 'var(--r-lg) var(--r-lg) 0 0', overflow: 'hidden', aspectRatio: '16/11' }}>
                ...
                <span style={{ position: 'absolute', top: 14, left: 14 }}><Verdict v={feat.verdict} /></span>
              ...
              <div style={{ padding: '16px 18px 18px' }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--gray)', marginTop: 4, display: 'inline-flex', alignItems: 'center', gap: 3 }}>
                  {isPlace && <Icon name="pin" size={12} style={{ color: 'var(--gray-light)' }} />}{isPlace ? `${feat.area} · ${feat.catLabel}` : feat.catLabel}
                </div>
            ...
            <Link href={`/p/${feat.id}`} className="btn btn-ink btn-block" style={{ marginTop: 10 }}>
...
function CardSection({ s, carousel }) {
  const items = s.ids.map((id) => GB.get(id)).filter(Boolean);
  return (
    <section className="section">
      <div className="wrap">
        <div className="section__head">
          ...
          <Link className="section__more" href="/category">전체 보기 <Icon name="chevR" size={15} /></Link>
        </div>
        <div className={carousel ? 'crow no-sb' : 'grid-cards'}>
    ...
    <section style={{ background: '#22252A', color: '#fff', marginTop: 20 }}>
      <div className="wrap section">
        <div className="section__head" style={{ alignItems: 'center' }}>
          ...
          <Link className="section__more" style={{ color: '#C9CDD3' }} href="/rejected">전체 탈락 리스트 <Icon name="chevR" size={15} /></Link>
        </div>
          ...
          {items.map((p) => (
            <Link key={p.id} href={`/p/${p.id}`}
              style={{ display: 'flex', gap: 16, textAlign: 'left', background: 'rgba(255,255,255,.04)',
                border: '1px solid rgba(255,255,255,.09)', borderRadius: 'var(--r-lg)', padding: 14 }}>
      ...
      <div className="wrap">
        <div style={{ background: 'linear-gradient(135deg, var(--green-tint), #fff)', border: '1px solid var(--line)',
          ...
          <div style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 16, marginBottom: 26 }}>
            ...
            <Link className="btn btn-soft" href="/verify">검증 기준 전체 보기 <Icon name="chevR" size={15} /></Link>
          </div>
...
export function HomePage() {
  ...
  return (
    <div>
      ...
      {secs.map((s, i) => (
        <CardSection key={s.id} s={s} carousel={i === secs.length - 1 && secs.length > 1} />
      ))}
      <RejectedTeaser />
      <VerifyCTA />
    </div>
